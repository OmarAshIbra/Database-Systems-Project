...
async function EditStatus(PNUMBER) {
    const db = await getDbConnection();
    const sql = `update Package SET  status = 'shipped'  where PNUMBER = ?`;
...
async function EditStatusR(PNUMBER) {
    const db = await getDbConnection();
    const sql = `update Package SET  status = 'delivered'  where PNUMBER = ?`;
...
async function Getconfomedpaymnts() {
    const db = await getDbConnection();
    const sql = `select * from package WHERE status = 'shipped' `;
...
module.exports = {
    addPackage,
    addShippedPackage, RemovePackage,EditStatus,EditStatusR,
